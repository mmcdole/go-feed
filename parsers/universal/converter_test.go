...
func TestDefaultRSSConverter_Translate(t *testing.T) {
...
func TestDefaultRSSConverter_Translate_WrongType(t *testing.T) {
	translator := &DefaultRSSConverter{}
...
func TestDefaultAtomConverter_Translate(t *testing.T) {
...
func TestDefaultAtomConverter_Translate_WrongType(t *testing.T) {
	translator := &DefaultAtomConverter{}
	af, err := translator.Translate("wrong type")
	assert.Nil(t, af)
	assert.NotNil(t, err)
...
func TestDefaultJSONConverter_Translate(t *testing.T) {
	files, _ := filepath.Glob("testdata/parser/translator/json/*.json")
	for _, f := range files {
		base := filepath.Base(f)
		name := strings.TrimSuffix(base, filepath.Ext(base))
		...
		// Get actual source feed
		ff := fmt.Sprintf("testdata/parser/translator/json/%s.json", name)
		f, _ := os.Open(ff)
		defer f.Close()

		// Parse actual feed
		translator := &DefaultJSONConverter{}
		fp := json.Parser{}
		jsonFeed, _ := fp.Parse(f)
		actual, _ := translator.Translate(jsonFeed)

		// Get json encoded expected feed result
		ef := fmt.Sprintf("testdata/parser/translator/json/%s_expected.json", name)
		e, _ := ioutil.ReadFile(ef)
...
func TestDefaultJSONConverter_Translate_WrongType(t *testing.T) {
	translator := &DefaultJSONConverter{}
	af, err := translator.Translate("wrong type")
	assert.Nil(t, af)
	assert.NotNil(t, err)
